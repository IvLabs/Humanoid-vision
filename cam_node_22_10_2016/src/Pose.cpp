...
Pose::Pose(int flag)
    {
        ...
        objectPoints1.push_back(cv::Point3f( -0.100f,0.100f,0.0f));
	objectPoints1.push_back(cv::Point3f(-0.100f,-0.100f,0.0f));
 	objectPoints1.push_back(cv::Point3f(0.100f,-0.100f,0.0f));
  	objectPoints1.push_back(cv::Point3f(0.100f,0.100f,0.0f));
  
        objectPoints2.push_back(cv::Point3f( 0.100f,0.100f,0.0f));
	objectPoints2.push_back(cv::Point3f(-0.100f,0.100f,0.0f));
 	objectPoints2.push_back(cv::Point3f(-0.100f,-0.100f,0.0f));
  	objectPoints2.push_back(cv::Point3f(0.100f,-0.100f,0.0f));
        }
        ...
        objectPoints1.push_back(cv::Point3f( -0.050f,0.050f,0.0f));
	objectPoints1.push_back(cv::Point3f(-0.050f,-0.050f,0.0f));
 	objectPoints1.push_back(cv::Point3f(0.050f,-0.050f,0.0f));
  	objectPoints1.push_back(cv::Point3f(0.050f,0.050f,0.0f));
  
        objectPoints2.push_back(cv::Point3f( 0.050f,0.050f,0.0f));
	objectPoints2.push_back(cv::Point3f(-0.050f,0.050f,0.0f));
 	objectPoints2.push_back(cv::Point3f(-0.050f,-0.050f,0.0f));
  	objectPoints2.push_back(cv::Point3f(0.050f,-0.050f,0.0f));
...
double* Pose::contour_detection(Mat threshold_canny,Mat threshold_ostu ,Mat gray_src, Mat src,int *blue_centroid)
{
   	...
   	int largest_contour_index=0;
        ...
        int j=0;
        int index[500];
        double area_array[500];  //area array
        int sum = 0;    
        double d;//distance
        int y=0;//sort
       int q=0;
 ...
 /* int p=j-1;
	if(j>2)
	{
		...
		{      
	                ...
	                if(contours[index[p]][1].y < contours[index[p]][3].y)
                        {
    			...
    			quad_pts[3]=Point(contours[index[p]][3].x,contours[index[p]][3].y); 
                        cout<<"c"<<endl;
                        squre_pts[0]=Point(boundRect.x+boundRect.width,boundRect.y);
			...
			quad_pts[0]=Point(contours[index[p]][0].x,contours[index[p]][0].y);
    		 ...
    		 cout<<""<<endl;
              ...
              cout<<"111111"<<endl;
    		 Mat transformed = Mat::zeros(threshold_extract.rows, threshold_extract.cols,CV_8UC1);	
              cout<<"222222"<<endl;
 		warpPerspective(threshold_extract, transformed, transmtx, threshold_extract.size());  
                   cout<<"333333"<<endl;          
	...
	//	{
	        ...
	        ///marker=threshold_extract(Rect(Point(contours[index[p]][0].x,contours[index[p]][0].y),Point(contours[index[p]][2].x,contours[index[p]][2].y)));
		...
		cv::Mat bitMatrix = cv::Mat::zeros(4,4,CV_8UC1);
              ...
              cout<<"cols : "<< marker.cols<<endl;
	      int height = marker.rows / 4;
              cout<<"height : "<< height<<endl;
               cout<<"rows : "<< marker.rows<<endl;
              Mat cell;
                      ...
                      
		{
			 ...
			 double ratio=area_array[j-s]/area_array[j-(r+1)];
              ...
              Rect boundRect=boundingRect(contours[index[q]]);
	                ...
	                if(contours[index[q]][1].y < contours[index[q]][3].y)
                        {
			...
			quad_pts[0]=Point(contours[index[q]][0].x,contours[index[q]][0].y);
			 ...
			 Mat transmtx = getPerspectiveTransform(quad_pts,squre_pts);  
                         ...
                         warpPerspective(threshold_ostu, transformed, transmtx, threshold_ostu.size());  
                   	 rectangle(transformed,boundRect,Scalar(0,255,0),1,8,0);
