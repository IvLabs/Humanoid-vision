...
//								   CV_FOURCC('P','I','M','1'),
	...
	if( !useCamera ) {
		draw_image = cvLoadImage( "/home/aniket/mak.jpg" );
		if(!draw_image){
...
//		draw_image    = cvCreateImage(cvSize (current_frame->width, current_frame->height), IPL_DEPTH_8U, 3);
		...
		{	
		/*Mat draw_image;
		bool bSuccess = cap.read(draw_image); // read a new frame from video
...
//			cvFlip (current_frame, draw_image, 1);
			...
			cvResize (current_frame, draw_image, CV_INTER_LINEAR);
			 Mat draw_image1(draw_image);
			
			// Perform a Gaussian blur
			//cvSmooth( draw_image, draw_image, CV_GAUSSIAN, 3, 3 );
                        vector<Mat> rgbChannels(3);
			split(draw_image1, rgbChannels);
			Mat g,new_rgb,diff_red,diff_blue,diff_green;
			g = Mat::ones(Size(draw_image1.cols, draw_image1.rows), CV_8UC1);

			diff_red=(rgbChannels[2]-rgbChannels[1])+g;
			diff_green=(rgbChannels[1]-rgbChannels[0])+g;
			diff_blue=(rgbChannels[0]-rgbChannels[2])+g; 
                        draw_image1=diff_blue > 43 & diff_blue < 50 & diff_green > 0 & diff_green <25 & diff_red> 0 & diff_red < 25;
			
			//IplImage* draw_image2=draw_image1; 
                        //IplImage* draw_image2=cvCloneImage(&(IplImage)draw_image1);

			IplImage copy =draw_image1 ;
 			IplImage* draw_image2 = &copy;			

			buffer->setBuffer( (unsigned char *) draw_image2->imageData, draw_image2->width, draw_image2->height);//**
			...
			// just show the image
			cvShowImage(WINDOW_NAME, draw_image2);//**
	
			if( writeVideo ) {
				cvWriteFrame(writer,draw_image2);  //**    // add the frame to the file
			}
